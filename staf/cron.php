    ...
    <table class="display" id="data_tbl_tools">
        ...
        while ($record = mysql_fetch_array($result)) {
            ...
            <tr class="">
                ...
                <td align="center"><? echo $record["masuk"];
                    $totalMasuk += $record["masuk"]; ?></td>
                <td align="left">
                    <div align="right"><? echo $record["keluar"];
                        $totalAmaun += $record["keluar"];
                        $bal += $record["masuk"] - $record["keluar"]; ?></div>
                </td>
                <td align="center"><strong><? echo number_format($bal, 2, ".", ","); ?></strong></td>
                <td bgcolor="#99FFFF">
                    <div align="center"><a href='' class="style27">Detail</a></div>
                </td>
            </tr>
            <?
            $i++;
        ...
        <tr>
            ...
            <th align="right"><strong><? echo number_format($totalAmaun, 2, ".", ",");
                    $baki = $totalMasuk - $totalAmaun; ?></strong></th>
...
$sql = "insert into t_topup (cawangan, butir, tarikh, masuk, keluar, noslip, masa, oleh, date2)
/*user current_balance instead*/
SELECT cawangan, 'Balance Forward', tarikh, current_balance, keluar, noslip, masa, oleh, now() FROM
(
SELECT *, sum(curr_balance) AS current_balance
FROM (SELECT *, masuk-keluar AS curr_balance FROM t_topup
ORDER BY idtopup DESC
) AS tmp_table GROUP BY cawangan
